-- voiced-mark-filter.lua - Spec
-- Purpose:
--   Convert a base character followed by a voiced sound mark (U+309B "゛") to LaTeX \dakuten{...}.
--   Only handles the standalone voiced mark, not combining dakuten.
-- Output target:
...
-- Voiced mark character constant
local VOICED_MARK = "\227\130\155"         -- U+309B "゛"
local UTF8_CHAR_PATTERN = "[\0-\x7F\xC2-\xF4][\x80-\xBF]*"

local function process_voiced_mark_text(text)
  if not text:find(VOICED_MARK) then
    return nil
  end

  local pattern = "(" .. UTF8_CHAR_PATTERN .. ")" .. VOICED_MARK
...
local function voiced_mark_Str(elem)
  return utils.process_str_element(elem, process_voiced_mark_text)
end

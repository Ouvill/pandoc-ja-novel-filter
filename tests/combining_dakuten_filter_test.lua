-- test_combining_dakuten_filter.lua
-- Unit tests for combining-dakuten-filter.lua (U+3099 only)
...
_G.pandoc = pandoc
local filter = dofile((... and (...):gsub("[^/\\]+$", "combining-dakuten-filter.lua")) or "combining-dakuten-filter.lua")
...
-- 1) No combining dakuten: unchanged (nil)
do
...
do
  ...
  assert_len(arr, 1, "Single combining dakuten: should return a single element")
  assert_equal(arr[1].t, "RawInline", "Single combining dakuten: element type should be RawInline")
  assert_equal(arr[1].format, "latex", "Single combining dakuten: RawInline format should be latex")
  assert_equal(arr[1].text, "\\dakuten{あ}", "Single combining dakuten: latex text mismatch")
end

-- 3) Voiced mark should NOT be processed (should return nil)
do
  local input = "あ" .. voiced
  local elem = make_elem(input)
  local out = Str(elem)
  assert_true(out == nil, "Voiced mark: should return nil (not processed)")
end

-- 4) Multiple combining dakuten: か + U+3099 + き + U+3099 -> \dakuten{か}\dakuten{き}
do
  local input = "か" .. combining_dakuten .. "き" .. combining_dakuten
  local elem = make_elem(input)
  local out = Str(elem)
  local arr = flatten(out)
  assert_len(arr, 2, "Multiple combining dakuten: should return two elements")
  assert_equal(arr[1].t, "RawInline", "Multiple combining dakuten: first element type")
  assert_equal(arr[1].text, "\\dakuten{か}", "Multiple combining dakuten: first latex text")
  assert_equal(arr[2].t, "RawInline", "Multiple combining dakuten: second element type")
  assert_equal(arr[2].text, "\\dakuten{き}", "Multiple combining dakuten: second latex text")
end

-- 5) Mixed with surrounding text: "A" .. あ + U+3099 .. "B"
do
  local input = "A" .. "あ" .. combining_dakuten .. "B"
  local elem = make_elem(input)
  local out = Str(elem)
  local arr = flatten(out)
...
-- 6) Precomposed kana (が) should be unchanged
do
...
-- 8) 4-byte UTF-8 base char + combining dakuten -> \dakuten{base}
do
  local base4 = "𠀋" -- U+2000B (4-byte in UTF-8)
  local input = base4 .. combining_dakuten
  local elem = make_elem(input)
  local out = Str(elem)
  local arr = flatten(out)
